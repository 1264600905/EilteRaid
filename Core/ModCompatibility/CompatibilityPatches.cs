    ...
    internal class CompatibilityPatches
    {
        ...
        private static bool PatchMSER(Harmony harmony)
        {
            ...
            if (MOD_MSER_Active)
            {
                ...
                {
                    Log.Error(String.Format("[Elite Raid] Error: [Medical System Expansion - Revived] Compatibility Patch Failed!! reason:{0}{1}", Environment.NewLine, ex.ToString()));
                }
            }
            return false;
        }
        public static bool OrenoMSE_MSE_VanillaExtender_HediffApplyHediffs_Prefix(Hediff hediff, Pawn pawn, BodyPartRecord record)
        {
            ...
            //}
            //return forward;
            return !(pawn?.health?.hediffSet?.hediffs.Where(x => x is CR_DummyForCompatibility).Any() ?? true);
        }
        #endregion
        ...
        private static bool PatchNoMoreRelative(Harmony harmony)
        {
            ...
            if (MOD_NoMoreRelationShip_Active)
            {
                ...
                {
                    Log.Error(String.Format("[Elite Raid] Error: [NoMoreRelationShip禁止袭击生成亲戚] Compatibility Patch Failed!! reason:{0}{1}", Environment.NewLine, ex.ToString()));
                }
            }
            return false;
